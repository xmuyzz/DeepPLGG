...
def t2w_template(temp_dir, interp_type='linear', brain_extraction=False):

    if brain_extraction:
        # brain extration
        input_img = os.path.join(temp_dir, 't2w_template.nii.gz')
        output_img = os.path.join(temp_dir, 't2w_brain.nii.gz')
        hd_bet(inputs=input_img, outputs=output_img, device='cpu', mode='fast', tta=0)

    # respace to (1, 1, 3)
    img = sitk.ReadImage(os.path.join(temp_dir, 't2w_head.nii.gz'), sitk.sitkFloat32)
    ...
    sitk.WriteImage(img, os.path.join(temp_dir, 'temp_head.nii.gz'))
    print('complete!')
